...
Carey Price had the highest goalie salary of \$10.5M, while Igor Shesterkin will earn \$11.85 million in '25-26.
...
goalies <- goalies %>%
  mutate(
    cap_quartile = cut(
      Cap_Hit,
      breaks = cap_hit_quartiles,
      include.lowest = TRUE,
      labels = c("Q1", "Q2", "Q3", "Q4")
    )
  )

# Step 2: Calculate median Cap_Hit for each quartile group
median_by_quartile <- goalies %>%
  group_by(cap_quartile) %>%
  summarise(median_Cap_Hit = median(Cap_Hit, na.rm = TRUE))

# View result
print(median_by_quartile)

goalies <- goalies %>%
  mutate(Sv_percent = as.numeric(gsub("[^0-9\\.]", "", Sv_percent)))

# Step 2: Calculate average Save Percentage
average_sv_percent <- mean(goalies$Sv_percent, na.rm = TRUE)

# View result
print(average_sv_percent)
...
## 4.4 Regression for Cap Hit against Salary Cap.
...
```{r redfine, echo=FALSE}
# Define the season salary caps
cap_by_season <- c(
  "2021-2022" = 81.5,
  "2022-2023" = 82.5,
  "2023-2024" = 83.5,
  "2024-2025" = 83.5  # Or 88.0 if you're using the upper cap
)

# Compute Cap Hit as % of team cap
goalies <- goalies %>%
  mutate(Cap_Percent = Cap_Hit / cap_by_season[Season])

# Create bins based on Cap_Percent
goalies <- goalies %>%
  mutate(Cap_Percent_Bin = cut(Cap_Percent,
                               breaks = quantile(Cap_Percent, probs = seq(0, 1, 0.25), na.rm = TRUE),
                               include.lowest = TRUE,
                               labels = c("Low", "Mid-Low", "Mid-High", "High")))
...
# Plot: Cap_Percent vs Cap_Percent_Bin
plot1 <- ggplot(goalies, aes(x = Cap_Percent_Bin, y = Cap_Percent)) +
  geom_boxplot(fill = "skyblue", alpha = 0.7) +
  labs(title = "Distribution of Cap % by Bin",
       x = "Cap % Group",
       y = "Cap Hit (% of Salary Cap)") +
  theme_minimal()

# Plot: Sv% vs Cap_Percent_Bin
plot2 <- ggplot(goalies, aes(x = Cap_Percent_Bin, y = Sv_percent)) +
  geom_boxplot(fill = "steelblue", alpha = 0.7) +
  labs(title = "Save % by Cap % Bin",
       x = "Cap % Group", y = "Sv%") +
  theme_minimal()

# Plot: Wins vs Cap_Percent_Bin
plot3 <- ggplot(goalies, aes(x = Cap_Percent_Bin, y = Wins)) +
  geom_boxplot(fill = "darkgreen", alpha = 0.7) +
  labs(title = "Wins by Cap % Bin",
       x = "Cap % Group", y = "Wins") +
  theme_minimal()

# Plot: TOI vs Cap_Percent_Bin
plot4 <- ggplot(goalies, aes(x = Cap_Percent_Bin, y = TOI_minutes)) +
  geom_boxplot(fill = "darkgreen", alpha = 0.7) +
  labs(title = "TOI by Cap % Bin",
       x = "Cap % Group", y = "Time on Ice (minutes)") +
  theme_minimal()

# Optionally show all side by side
...
#### 4.4.1 Prediction Regression Model
...
inter_dollars <- intercept * 83875000
...
#### 4.4.2 Test Model for Potential Problems
...
### 4.4.1 Regression with Only TOI against Cap Hit

```{r regTOI, echo=FALSE, include=FALSE, message=FALSE}

model_caphit_toi <- lm(Cap_Percent ~ TOI_minutes, data = goalies)

summary(model_caphit_toi)

coef_values <- coef(model_caphit_toi)
...
```{r regtoi_plot, echo=FALSE, warning=FALSE, message=FALSE}
goalies$Predicted_Cap_Hit <- predict(model_caphit_toi)
...
```{r reg_toi_resid_plots, echo = FALSE, message = FALSE}

par(mfrow = c(2, 2))

plot(model_caphit_toi)
```
...
Further research would include more variables in selection, i.e. does it matter which hand a goalie catches and does that lead to better performance. Can also look when a goalie gets traded and receives a pay increase or decrease. Would also look at best performing goalie each year (The Vezina Trophy) and track that and see how that affects their pay when it comes to contract year. Could also track contracts and see when they change, what led to that.
